...
export default function DashboardScreen() {
  ...
  const refreshApp = useCallback(async () => {
    ...
    setRefreshing(true);
    setRefreshText("Synchronisation des matchs…");
    try {
      setMatches(await getMatches());
      if (!Updates.isEnabled) {
        setRefreshText("Matchs actualisés");
        return;
      }
      setRefreshText("Recherche d'une mise à jour…");
      const update = await Updates.checkForUpdateAsync();
      if (!update.isAvailable) {
        setRefreshText("Application et matchs actualisés");
        return;
      }
      setRefreshText("Téléchargement de la mise à jour…");
      await Updates.fetchUpdateAsync();
      Alert.alert("Mise à jour prête", "L'application va redémarrer.", [
        { text: "Redémarrer", onPress: () => Updates.reloadAsync() },
      ]);
    } catch {
      setRefreshText("Actualisation impossible");
      Alert.alert("Actualisation", "Impossible d'actualiser les données pour le moment.");
    } finally {
      setRefreshing(false);
    ...
    <SafeAreaView style={styles.container}>
      <ImageBackground source={marbleSource} resizeMode="cover" style={styles.background} imageStyle={styles.backgroundImage}>
        ...
        <ScrollView
          ...
          refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refreshApp} tintColor={Theme.colors.goldLight} colors={[Theme.colors.gold]} progressBackgroundColor="#151515" title={refreshText} titleColor={Theme.colors.goldLight} />}
        >
          ...
          <View style={styles.header}>
            ...
            <View style={styles.brand}>
              ...
              <View><Text style={styles.brandName}>DYNO</Text><Text style={styles.brandSub}>ESPORT MANAGER</Text></View>
            </View>
            <TouchableOpacity style={styles.coachBadge} activeOpacity={0.8}>
              <Ionicons name="diamond" size={13} color={Theme.colors.goldLight} />
              <Text style={styles.coachLetter}>C</Text><Text style={styles.coachRole}>Coach</Text>
            </TouchableOpacity>
          </View>
          ...
          <View style={styles.heroCard}>
            ...
            <View style={styles.heroTop}>
              <View style={styles.dateBadge}><Text style={styles.dateMonth}>{dateParts.month}</Text><Text style={styles.dateDay}>{dateParts.day}</Text></View>
              <View><Text style={styles.eyebrow}>{nextMatch ? `PROCHAIN ${nextMatch.type.toUpperCase()}` : "AUCUN MATCH PRÉVU"}</Text><Text style={styles.confirmedSmall}>{nextMatch?.status.toUpperCase() ?? "À PLANIFIER"}</Text></View>
            </View>
            ...
            <View style={styles.metaRow}>
              <Meta icon="time-outline" value={formatTime(nextMatch?.arrivalTime)} label="Lobby" /><View style={styles.divider} />
              <Meta icon="locate-outline" value={formatTime(nextMatch?.matchTime)} label="Début du match" /><View style={styles.divider} />
          ...
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeader}><Text style={styles.sectionTitle}>ANNONCE MATCH</Text><Text style={styles.seeAll}>{nextMatch?.status ?? "Aucune"}</Text></View>
...
function Meta({ icon, value, label }: { icon: keyof typeof Ionicons.glyphMap; value: string; label: string }) {
  return <View style={styles.metaItem}><Ionicons name={icon} size={22} color={Theme.colors.goldLight} /><Text style={styles.metaValue}>{value}</Text><Text style={styles.metaLabel}>{label}</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  coachLetter: { color: "#fff", fontSize: 22, fontWeight: "900" }, coachRole: { color: "#D0D0D0", fontSize: 10, fontWeight: "700" },
  ...
  heroTop: { flexDirection: "row", alignItems: "center" }, dateBadge: { width: 64, height: 74, borderRadius: 17, backgroundColor: "#F5F1E8", alignItems: "center", justifyContent: "center", marginRight: 16 },
  dateMonth: { color: "#725C1D", fontSize: 12, fontWeight: "900" }, dateDay: { color: "#111", fontSize: 30, lineHeight: 32, fontWeight: "900" }, eyebrow: { color: Theme.colors.goldLight, fontSize: 15, fontWeight: "900", letterSpacing: 1.1 }, confirmedSmall: { color: "#91D653", fontSize: 12, fontWeight: "900", marginTop: 6 },
  versusRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginVertical: 28 }, teamName: { color: "#fff", fontSize: 27, fontWeight: "900", letterSpacing: 1, maxWidth: "42%" }, vs: { color: Theme.colors.gold, fontSize: 19, fontWeight: "900" },
  metaRow: { flexDirection: "row", alignItems: "center", borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.13)", paddingTop: 17 }, metaItem: { flex: 1, alignItems: "center" }, metaValue: { color: "#fff", fontSize: 14, fontWeight: "900", marginTop: 6 }, metaLabel: { color: "#D1D1D1", fontSize: 10, textAlign: "center", marginTop: 3 }, divider: { width: 1, height: 54, backgroundColor: "rgba(255,255,255,0.13)" },
  confirmButton: { alignSelf: "center", minWidth: 190, height: 48, borderRadius: 24, marginTop: 19, flexDirection: "row", gap: 8, alignItems: "center", justifyContent: "center", backgroundColor: "rgba(4,4,4,0.88)", borderWidth: 1, borderColor: "rgba(224,184,67,0.5)" }, confirmText: { color: "#92DD54", fontSize: 17, fontWeight: "900" },
