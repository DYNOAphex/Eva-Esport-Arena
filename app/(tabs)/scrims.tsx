...
export default function ScrimsScreen() {
  ...
  async function handleSave() {
    ...
    if (!title) {
      setFormError("Renseigne l'équipe adverse avant de programmer le scrim.");
      return;
    }
    if (!validDate) {
      setFormError("Sélectionne une date avant de programmer le rendez-vous.");
      return;
    }
    if (!validMatchTime) {
      setFormError("Choisis l'heure du match : 21h20 ou 22h00.");
      return;
    }
    ...
    try {
      ...
      if (!editId) {
        await notifyMatchCreated({ type: match.type, opponent: match.opponent, date: match.date, matchTime: match.matchTime, arena: match.arena }).catch(() => null);
      ...
      Alert.alert(successTitle, `${match.opponent} est enregistré pour ${formatTime(match.matchTime)}.`, [
      ...
      <ImageBackground source={marbleSource} style={styles.background} imageStyle={styles.backgroundImage}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.formCard}>
            ...
            <TextInput
              style={[styles.input, formError && missingOpponent && styles.fieldError]}
              placeholder={isReplay ? "Ex. Analyse VOD / Travail défensif" : "Ex. TITANS"}
              placeholderTextColor="#777"
              value={opponent}
              onChangeText={(value) => { setOpponent(value); setFormError(""); }}
              autoCapitalize={isReplay ? "sentences" : "characters"}
            />

            <Label text="Date" />
            <DateField value={date} error={Boolean(formError && !validDate)} onChange={(value) => { setDate(value); setFormError(""); }} />
              ...
              {matchTimes.map((value) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={[styles.timeChoice, active && styles.timeChoiceActive, formError && !validMatchTime && styles.fieldError]}
                    onPress={() => { setMatchTime(value); setFormError(""); }}
                  >
            ...
            <Text style={styles.timeHint}>Deux créneaux disponibles pour les matchs DYNO.</Text>
          ...
          <ScrimFormSummary type={type} opponent={opponent} date={date} matchTime={matchTime} arena={isReplay ? "Aucune" : arena} status={status} isReplay={isReplay} />

          {formError ? (
            <View style={styles.errorBanner}>
              <Ionicons name="alert-circle" size={19} color="#FF9B9B" />
              <Text style={styles.errorText}>{formError}</Text>
            </View>
          ) : null}

          <TouchableOpacity style={[styles.saveButton, saving && styles.disabled]} disabled={saving} onPress={() => void handleSave()}>
            <Ionicons name="save-outline" size={19} color="#080808" />
...
function DateField({ value, onChange, error }: { value: string; onChange: (value: string) => void; error?: boolean }) {
  if (Platform.OS === "web") {
    return createElement("input", {
      ...
      style: { ...webInputStyle, border: error ? "1px solid rgba(255,110,110,0.8)" : webInputStyle.border },
  ...
  return (
    <View>
      <TouchableOpacity style={[styles.selector, error && styles.fieldError]} onPress={() => setVisible(true)}>
...
const styles = StyleSheet.create({
  ...
  fieldError: { borderColor: "rgba(255,110,110,0.82)", borderWidth: 1 },
  ...
  timeChoice: { flex: 1, minHeight: 58, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, backgroundColor: "rgba(255,255,255,0.045)", borderWidth: StyleSheet.hairlineWidth, borderColor: "rgba(255,255,255,0.14)" },
  timeChoiceActive: { backgroundColor: Theme.colors.goldLight, borderColor: Theme.colors.goldLight },
  timeChoiceText: { color: "#F0F0F0", fontSize: 17, fontWeight: "900" },
  timeChoiceTextActive: { color: "#080808" },
  timeHint: { color: "#8F8F8F", fontSize: 9, fontWeight: "700", marginTop: 7 },
  previewLabel: { color: Theme.colors.goldLight, fontSize: 10, fontWeight: "900", letterSpacing: 1.2, marginTop: 22, marginBottom: 9 },
  errorBanner: { minHeight: 50, borderRadius: 16, paddingHorizontal: 14, paddingVertical: 11, flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "rgba(140,30,30,0.16)", borderWidth: StyleSheet.hairlineWidth, borderColor: "rgba(255,110,110,0.38)", marginBottom: 10 },
  errorText: { flex: 1, color: "#FFD0D0", fontSize: 11, lineHeight: 16, fontWeight: "800" },
