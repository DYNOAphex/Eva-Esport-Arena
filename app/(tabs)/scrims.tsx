...
export default function ScrimsScreen() {
  ...
  async function handleSave() {
    ...
    if (!title || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      Alert.alert("Informations incomplètes", isReplay ? "Indique une date pour le rendez-vous." : "Indique l'équipe adverse et sélectionne une date.");
      return;
    }
    try {
      if (!editId && Platform.OS === "web") await requestNotificationPermission().catch(() => false);
      setSaving(true);
      const selectedTime = isReplay ? matchTime : arrivalTime;
      const input = { type, opponent: title, date, arrivalTime: selectedTime, matchTime, arena: isReplay ? "Aucune" as const : arena, status, notes: notes.trim() };
      const match = editId ? await updateMatch(editId, input) : await createMatch(input);
      if (!editId) {
        await notifyMatchCreated({ type: match.type, opponent: match.opponent, date: match.date, arrivalTime: match.arrivalTime, matchTime: match.matchTime, arena: match.arena }).catch(() => null);
      ...
      const successTitle = isReplay ? (editId ? "Rendez-vous modifié" : "Rendez-vous programmé") : (editId ? "Scrim modifié" : "Scrim programmé");
      Alert.alert(successTitle, `${match.opponent} est enregistré.`, [{ text: "Voir l'Agenda", onPress: () => router.replace("/(tabs)/planning") }]);
    } catch (error) {
      Alert.alert("Erreur", error instanceof Error ? error.message : "Le rendez-vous n'a pas pu être enregistré.");
  ...
  return <SafeAreaView style={styles.container}><ImageBackground source={marbleSource} style={styles.background} imageStyle={styles.backgroundImage}><View style={styles.overlay} /><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}><Text style={styles.kicker}>DYNO ESPORT MANAGER</Text><Text style={styles.title}>{editId ? "Modifier le rendez-vous" : "Programmer un rendez-vous"}</Text><Text style={styles.subtitle}>{editId ? "Mets à jour les informations." : "Ajoute un scrim ou une session replay / strat à l'agenda."}</Text><View style={styles.formCard}><Label text="Type de rendez-vous" /><View style={styles.chipRow}>{appointmentTypes.map((value) => <TouchableOpacity key={value} style={[styles.chip, type === value && styles.chipActive]} onPress={() => chooseType(value)}><Text style={[styles.chipText, type === value && styles.chipTextActive]}>{value}</Text></TouchableOpacity>)}</View><Label text={isReplay ? "Titre du rendez-vous" : "Équipe adverse"} /><TextInput style={styles.input} placeholder={isReplay ? "Ex. Analyse VOD / Travail défensif" : "Ex. TITANS"} placeholderTextColor="#777" value={opponent} onChangeText={setOpponent} autoCapitalize={isReplay ? "sentences" : "characters"} /><Label text="Date" /><DateField value={date} onChange={setDate} />{isReplay ? <><Label text="Heure" /><ClockField value={matchTime} onChange={(value) => { setMatchTime(value); setArrivalTime(value); }} /></> : <><Label text="Heure de rendez-vous" /><ClockField value={arrivalTime} onChange={setArrivalTime} /><Label text="Heure du match" /><ClockField value={matchTime} onChange={setMatchTime} /><Label text="Arène" /><View style={styles.chipRow}>{arenas.map((value) => <TouchableOpacity key={value} style={[styles.chip, arena === value && styles.chipActive]} onPress={() => setArena(value)}><Text style={[styles.chipText, arena === value && styles.chipTextActive]}>{value}</Text></TouchableOpacity>)}</View></>}<Label text="Statut" /><View style={styles.statusWrap}>{statuses.map((value) => <TouchableOpacity key={value} style={[styles.chip, status === value && styles.chipActive]} onPress={() => setStatus(value)}><Text style={[styles.chipText, status === value && styles.chipTextActive]}>{value}</Text></TouchableOpacity>)}</View><Label text="Notes (optionnel)" /><TextInput style={[styles.input, styles.notes]} placeholder={isReplay ? "Objectifs, maps ou points à revoir" : "Informations utiles"} placeholderTextColor="#777" value={notes} onChangeText={setNotes} multiline /></View><Text style={styles.previewLabel}>VÉRIFICATION AVANT ENREGISTREMENT</Text><ScrimFormSummary type={type} opponent={opponent} date={date} arrivalTime={arrivalTime} matchTime={matchTime} arena={isReplay ? "Aucune" : arena} status={status} isReplay={isReplay} /><TouchableOpacity style={[styles.saveButton, saving && styles.disabled]} disabled={saving} onPress={() => void handleSave()}><Ionicons name="save-outline" size={19} color="#080808" /><Text style={styles.saveText}>{saving ? "ENREGISTREMENT…" : editId ? "ENREGISTRER LES MODIFICATIONS" : isReplay ? "PROGRAMMER LE RENDEZ-VOUS" : "PROGRAMMER LE SCRIM"}</Text></TouchableOpacity></ScrollView></ImageBackground></SafeAreaView>;
}

function Label({ text }: { text: string }) { return <Text style={styles.label}>{text}</Text>; }
function DateField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  if (Platform.OS === "web") return createElement("input", { type: "date", value, min: todayValue(), onChange: (event: { target: { value: string } }) => onChange(event.target.value), style: webInputStyle, "aria-label": "Date du rendez-vous" }) as never;
  const [visible, setVisible] = useState(false);
  const current = value && /^\d{4}-\d{2}-\d{2}$/.test(value) ? new Date(`${value}T12:00:00`) : new Date();
  function handleChange(event: DateTimePickerEvent, selected?: Date) { if (Platform.OS === "android") setVisible(false); if (event.type === "dismissed" || !selected) return; onChange(`${selected.getFullYear()}-${String(selected.getMonth() + 1).padStart(2, "0")}-${String(selected.getDate()).padStart(2, "0")}`); }
...
function ClockField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  if (Platform.OS === "web") return createElement("input", { type: "time", value, step: 300, onChange: (event: { target: { value: string } }) => onChange(event.target.value), style: webInputStyle, "aria-label": "Heure du rendez-vous" }) as never;
  const [visible, setVisible] = useState(false);
