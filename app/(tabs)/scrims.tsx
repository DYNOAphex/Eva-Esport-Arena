...
function ClockField({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  if (Platform.OS === "web") return createElement("input", { type: "time", value, step: 300, onChange: (event: { target: { value: string } }) => onChange(event.target.value), style: webInputStyle, "aria-label": "Heure du scrim" }) as never;
  const [visible, setVisible] = useState(false);
