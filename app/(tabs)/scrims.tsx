...
export default function ScrimsScreen() {
  ...
  async function handleCreateMatch() {
    if (!opponent.trim() || !/^\d{4}-\d{2}-\d{2}$/.test(date)) {
      Alert.alert("Informations incomplètes", "Choisis une date et indique l'équipe adverse.");
      return;
    }
            ...
            return (
              <View key={match.id} style={styles.card}>
                <View style={styles.cardTopRow}>
                  ...
                  <TouchableOpacity onLongPress={() => confirmDelete(match)}>
                    <Ionicons name="ellipsis-horizontal" size={23} color="#ddd" />
                  </TouchableOpacity>
                </View>
            ...
            <View style={styles.modalCard}>
              <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
                <View style={styles.modalHeader}>
                  <View>
                    <Text style={styles.kicker}>NOUVEL ÉVÉNEMENT</Text>
  ...
  return (
    <View>
      <TouchableOpacity style={styles.selector} onPress={() => setVisible(true)}>
        ...
        <Text style={styles.clockValue}>{value.replace(":", "h")}</Text>
        <Text style={styles.clockHint}>Toucher pour ouvrir l'horloge</Text>
        <Ionicons name="chevron-forward" size={18} color="#999" />
      </TouchableOpacity>
      ...
      {visible ? (
        <View style={Platform.OS === "ios" ? styles.iosPickerWrap : undefined}>
          <DateTimePicker
            ...
            minuteInterval={5}
            themeVariant="dark"
            accentColor={Theme.colors.goldLight}
            onChange={handleChange}
          />
          {Platform.OS === "ios" ? (
            <TouchableOpacity style={styles.pickerDone} onPress={() => setVisible(false)}>
...
function Info({ icon, label, value }: { icon: keyof typeof Ionicons.glyphMap; label: string; value: string }) {
  return <View style={styles.infoItem}><Ionicons name={icon} size={19} color={Theme.colors.goldLight} /><View><Text style={styles.infoLabel}>{label}</Text><Text style={styles.infoValue}>{value}</Text></View></View>;
}

function Label({ text }: { text: string }) { return <Text style={styles.label}>{text}</Text>; }

function ChipRow({ values, selected, onSelect }: { values: string[]; selected: string; onSelect: (value: string) => void }) {
  return <View style={styles.chipRow}>{values.map((value) => <TouchableOpacity key={value} style={[styles.chip, selected === value && styles.chipActive]} onPress={() => onSelect(value)}><Text style={[styles.chipText, selected === value && styles.chipTextActive]}>{value}</Text></TouchableOpacity>)}</View>;
}
...
function formatTime(value: Date) { return `${value.getHours().toString().padStart(2, "0")}:${value.getMinutes().toString().padStart(2, "0")}`; }
function timeToDate(value: string) { const [hours, minutes] = value.split(":").map(Number); return new Date(2000, 0, 1, hours || 0, minutes || 0); }
function toDateKey(value: Date) { return `${value.getFullYear()}-${(value.getMonth() + 1).toString().padStart(2, "0")}-${value.getDate().toString().padStart(2, "0")}`; }
function buildCalendar(cursor: Date) { const first = new Date(cursor.getFullYear(), cursor.getMonth(), 1); const last = new Date(cursor.getFullYear(), cursor.getMonth() + 1, 0); const offset = (first.getDay() + 6) % 7; return [...Array(offset).fill(null), ...Array.from({ length: last.getDate() }, (_, index) => new Date(cursor.getFullYear(), cursor.getMonth(), index + 1))]; }
...
const styles = StyleSheet.create({
  ...
  headerText: { flex: 1 },
  kicker: { color: Theme.colors.goldLight, fontSize: 10, fontWeight: "900", letterSpacing: 1.6 },
  title: { color: "#fff", fontSize: 34, fontWeight: "900" },
  subtitle: { color: "#ccc", marginTop: 6 },
  addButton: { width: 54, height: 54, borderRadius: 27, alignItems: "center", justifyContent: "center", backgroundColor: Theme.colors.goldLight },
  card: { padding: 18, marginBottom: 16, borderRadius: 25, backgroundColor: "rgba(8,8,8,0.78)", borderWidth: 1, borderColor: "rgba(224,184,67,0.42)" },
  cardTopRow: { flexDirection: "row", justifyContent: "space-between" },
  typeBadge: { flexDirection: "row", gap: 6, alignItems: "center", backgroundColor: Theme.colors.goldLight, paddingHorizontal: 11, paddingVertical: 7, borderRadius: 14 },
  typeBadgeText: { color: "#080808", fontWeight: "900" },
  opponent: { color: "#fff", fontSize: 23, fontWeight: "900", marginTop: 18 },
  vs: { color: Theme.colors.gold },
  infoGrid: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginTop: 18, paddingTop: 15, borderTopWidth: 1, borderTopColor: "rgba(255,255,255,0.12)" },
  infoItem: { width: "48%", flexDirection: "row", gap: 9, marginBottom: 14 },
  infoLabel: { color: "#999", fontSize: 10, textTransform: "uppercase" },
  infoValue: { color: "#fff", fontWeight: "800", marginTop: 2 },
  sectionTitle: { color: Theme.colors.goldLight, fontSize: 11, fontWeight: "900", letterSpacing: 1.1, marginTop: 10, marginBottom: 9 },
  answerRow: { flexDirection: "row", gap: 10 },
  answerButton: { flex: 1, minHeight: 48, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, borderWidth: 1, borderColor: "rgba(255,255,255,0.16)" },
  answerText: { color: "#fff", fontWeight: "800", fontSize: 12 },
  answerTextActive: { color: "#080808" },
  availableActive: { backgroundColor: "#82DD55", borderColor: "#82DD55" },
  unavailableActive: { backgroundColor: "#FF7777", borderColor: "#FF7777" },
  responseGroup: { backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 14, padding: 11, marginBottom: 8 },
  responseHeader: { flexDirection: "row", alignItems: "center", gap: 6 },
  responseTitle: { fontWeight: "900", fontSize: 11 },
  responseNames: { color: "#ddd", fontSize: 11, marginTop: 6, lineHeight: 17 },
  calendarButton: { minHeight: 50, backgroundColor: Theme.colors.goldLight, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 10 },
  calendarButtonText: { color: "#080808", fontWeight: "900", fontSize: 12 },
  modalBackdrop: { flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.72)" },
  modalBackdropCenter: { flex: 1, justifyContent: "center", padding: 20, backgroundColor: "rgba(0,0,0,0.76)" },
  modalCard: { maxHeight: "92%", backgroundColor: "#0B0B0B", borderTopLeftRadius: 30, borderTopRightRadius: 30, borderWidth: 1, borderColor: "rgba(224,184,67,0.5)", padding: 20 },
  modalHeader: { flexDirection: "row", justifyContent: "space-between", marginBottom: 12 },
  modalTitle: { color: "#fff", fontSize: 27, fontWeight: "900" },
  label: { color: "#ddd", fontSize: 12, fontWeight: "800", marginTop: 14, marginBottom: 8 },
  input: { minHeight: 54, borderRadius: 16, backgroundColor: "#171717", borderWidth: 1, borderColor: "rgba(224,184,67,0.25)", color: "#fff", paddingHorizontal: 14 },
  notesInput: { minHeight: 82, textAlignVertical: "top", paddingTop: 14 },
  selector: { minHeight: 58, borderRadius: 16, backgroundColor: "#171717", borderWidth: 1, borderColor: "rgba(224,184,67,0.25)", paddingHorizontal: 14, flexDirection: "row", alignItems: "center", gap: 10 },
  selectorText: { flex: 1, color: "#fff" },
  clockValue: { color: "#fff", fontSize: 22, fontWeight: "900" },
  clockHint: { flex: 1, color: "#9B9B9B", fontSize: 11 },
  iosPickerWrap: { marginTop: 8, padding: 10, borderRadius: 16, backgroundColor: "#151515" },
  pickerDone: { minHeight: 42, borderRadius: 13, backgroundColor: Theme.colors.goldLight, alignItems: "center", justifyContent: "center", marginTop: 8 },
  pickerDoneText: { color: "#080808", fontWeight: "900" },
  ...
  chipActive: { backgroundColor: Theme.colors.goldLight },
  chipText: { color: "#ddd", fontWeight: "800" },
  chipTextActive: { color: "#080808" },
  saveButton: { minHeight: 58, borderRadius: 18, backgroundColor: Theme.colors.goldLight, alignItems: "center", justifyContent: "center", marginTop: 24 },
  saveText: { color: "#080808", fontWeight: "900" },
  calendarCard: { backgroundColor: "#0B0B0B", borderRadius: 24, padding: 18, borderWidth: 1, borderColor: "rgba(224,184,67,0.5)" },
  calendarHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  calendarMonth: { color: "#fff", fontWeight: "900", textTransform: "capitalize" },
  weekRow: { flexDirection: "row", marginTop: 18 },
  weekDay: { width: "14.28%", color: Theme.colors.goldLight, textAlign: "center", fontWeight: "900" },
  ...
  closeCalendar: { marginTop: 12, minHeight: 46, borderRadius: 14, backgroundColor: Theme.colors.goldLight, alignItems: "center", justifyContent: "center" },
  closeCalendarText: { color: "#080808", fontWeight: "900" },
});
