...
export function toMatchDate(match: Pick<Match, "date" | "matchTime" | "matchTimeAlt">) { const time = match.matchTimeAlt && match.matchTimeAlt > match.matchTime ? match.matchTimeAlt : match.matchTime; const value = new Date(`${match.date}T${time}:00`); return Number.isNaN(value.getTime()) ? null : value; }
