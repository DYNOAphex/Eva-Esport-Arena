...
async function fetchCloudRoster() {
  ...
  if (seeded.length) await Promise.all(seeded.map((player) => firestoreRequest(`${FIRESTORE_BASE}/${encodeURIComponent(player.id)}`, { method: "DELETE" })));
  return sortPlayers(removeLegacySeeds(players));
}
async function uploadPlayer(player: RosterPlayer) {
  const response = await firestoreRequest(`${FIRESTORE_BASE}/${encodeURIComponent(player.id)}`, { method: "PATCH", body: JSON.stringify({ fields: fields(player) }) });
  if (!response.ok) throw new Error("Enregistrement du joueur impossible.");
}
async function syncFromCloud() {
  if (syncInProgress) return latestPlayers;
  syncInProgress = true;
  try { return persist(await fetchCloudRoster()); }
  catch { const local = await readStoredPlayers(); latestPlayers = local; return local; }
...
export function subscribeToRoster(listener: (players: RosterPlayer[]) => void) {
  listeners.add(listener); listener(latestPlayers); void syncFromCloud();
