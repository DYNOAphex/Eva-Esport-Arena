...
function mergePlayers(local: RosterPlayer[], cloud: RosterPlayer[]) {
  ...
  local.forEach((player) => {
    ...
    merged.set(player.id, current ? { ...player, ...current } : player);
  });
  return sortPlayers(removeLegacySeeds([...merged.values()]));
}
...
async function readStoredPlayers() {
  ...
  try {
    ...
    return sortPlayers(removeLegacySeeds(stored.map((player) => ({
      id: player.id,
      nickname: player.nickname,
      createdAt: player.createdAt,
      accountUid: typeof player.accountUid === "string" && player.accountUid ? player.accountUid : undefined,
      accountEmail: typeof player.accountEmail === "string" && player.accountEmail ? player.accountEmail : undefined,
    }))));
  } catch {
...
function fields(player: RosterPlayer): Record<string, FirestoreValue> {
  return {
    ...
    accountUid: { stringValue: player.accountUid ?? "" },
    accountEmail: { stringValue: player.accountEmail ?? "" },
  };
}
...
function documentToPlayer(document: FirestoreDocument): RosterPlayer {
  ...
  return {
    ...
    accountUid: readString(data.accountUid) || undefined,
    accountEmail: readString(data.accountEmail) || undefined,
  };
}
...
async function fetchCloudRoster() {
  ...
  return sortPlayers(removeLegacySeeds((data.documents ?? []).map(documentToPlayer)));
}
...
async function syncFromCloud() {
  ...
  try {
    ...
    const merged = mergePlayers(local, cloud);
    const missingInCloud = merged.filter((player) => !cloud.some((item) => item.id === player.id));
    await Promise.all(missingInCloud.map((player) => uploadPlayer(player).catch(() => null)));
...
export async function addRosterPlayer(input: Pick<RosterPlayer, "nickname">) {
  ...
  void uploadPlayer(player).catch(() => null);
  return player;
}
export async function ensureCurrentAccountRosterPlayer(nicknameInput: string) {
  ...
  const next = sameNickname ? players.map((item) => item.id === sameNickname.id ? player : item) : [...players, player];
  await persist(next);
  void uploadPlayer(player).catch(() => null);
  return player;
}
export async function linkCurrentAccountToPlayer(playerId: string) {
  ...
  await persist(players.map((player) => player.id === playerId ? changed : player));
  void uploadPlayer(changed).catch(() => null);
  return changed;
}
export async function unlinkPlayerAccount(playerId: string) {
  ...
  const target = players.find((player) => player.id === playerId);
  if (!target) throw new Error("Ce joueur n'existe plus.");
  const changed = { ...target, accountUid: undefined, accountEmail: undefined };
  await persist(players.map((player) => player.id === playerId ? changed : player));
  void uploadPlayer(changed).catch(() => null);
}
export async function getRosterPlayerForAccount(uid: string, email?: string) {
  const players = await readStoredPlayers();
  const normalizedEmail = email?.trim().toLowerCase();
  return players.find((player) => player.accountUid === uid) ?? players.find((player) => normalizedEmail && player.accountEmail?.toLowerCase() === normalizedEmail) ?? null;
}
export async function deleteRosterPlayer(playerId: string) {
  await requireUser();
  const players = await readStoredPlayers();
  await persist(players.filter((player) => player.id !== playerId));
  void firestoreRequest(`${FIRESTORE_BASE}/${encodeURIComponent(playerId)}`, { method: "DELETE" }).catch(() => null);
}
